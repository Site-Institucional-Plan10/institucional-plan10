  ...
  return (
    <section className="so" aria-labelledby="so-h">
      <style>{`
        ...
        .so-cat { font-family: 'JetBrains Mono', ui-monospace, monospace; font-size: .6rem; letter-spacing: .12em; text-transform: uppercase; color: #6B7280; display: block; margin-top: 4px; }
        ...
        @media (max-width: 780px) {
          .so { padding: 44px 20px; }
          .so-row { position: relative; grid-template-columns: 1fr; gap: 3px; padding: 12px 34px 12px 6px; }
          .so-desc { grid-column: auto; -webkit-line-clamp: 2; font-size: .86rem; }
          /* seta no canto superior direito, fora do fluxo */
          .so-cta { position: absolute; right: 4px; top: 12px; }
          .so-row:hover { padding-left: 6px; }
        }
      `}</style>
      <div className="so-in">
